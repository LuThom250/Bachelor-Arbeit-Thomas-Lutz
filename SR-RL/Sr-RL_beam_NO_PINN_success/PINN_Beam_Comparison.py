...
def plot_solutions(x_values, pinn_solution, analytical_solution):
    """Plottet den Vergleich zwischen PINN-Lösung und analytischer Lösung."""
    plt.figure(figsize=(12, 7))
    plt.plot(x_values, analytical_solution, 'k-', linewidth=3, label='Analytische Lösung')
    plt.plot(x_values, pinn_solution, 'r--', marker='o', markersize=5, label='PINN-Lösung')
    plt.title('Vergleich der Biegelinie: PINN vs. Analytisch', fontsize=16)
    plt.xlabel('Position auf dem Balken x [m]', fontsize=12)
    plt.ylabel('Durchbiegung u(x) [m]', fontsize=12)
    plt.grid(True, which='both', linestyle='--')
    plt.legend()
    plt.show()

def plot_training_loss(error_log):
    """Plottet den Verlauf der Verlustfunktion während des Trainings."""
    epochs, errors = zip(*error_log)
    plt.figure(figsize=(12, 7))
    plt.plot(epochs, errors)
    plt.yscale('log')
    plt.title('Trainingsverlauf (Verlustfunktion)', fontsize=16)
    plt.xlabel('Epoche', fontsize=12)
    plt.ylabel('Verlust (log-Skala)', fontsize=12)
    plt.grid(True, which='both', linestyle='--')
    plt.show()

def plot_absolute_error(x_values, pinn_solution, analytical_solution):
    """Plottet den absoluten Fehler zwischen der PINN-Lösung und der analytischen Lösung."""
    # Calculate the absolute error
    error = np.abs(np.array(pinn_solution) - np.array(analytical_solution))

    # Create the plot
    plt.figure(figsize=(12, 6))
    plt.plot(x_values, error, 'b-') # Blue solid line
    plt.title('Absoluter Fehler: PINN vs. Analytisch', fontsize=16)
    plt.xlabel('Position auf dem Balken x [m]', fontsize=12)
    plt.ylabel('Absoluter Fehler [m]', fontsize=12)
