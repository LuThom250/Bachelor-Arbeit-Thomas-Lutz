...
I = 40e-6    # Flächenträgheitsmoment in m^4
q = 60e3    # Basis-Lastparameter q_0
            # Physikalische Bedeutung je nach Lastfall:
            # - 'constant', 'sinus': Gleichlast/Amplitude in [N/m]
            # - 'linear':             Lastanstieg in [N/m^2]
            # - 'quadratic':          Lastkrümmung in [N/m^3]
...
# -------------------------------------------------
MAX_LENGTH    = 23   # Etwas mehr Länge für das Polynom 4. Grades
X_RANGE       = (0, L)
N_POINTS      = 50
EPISODES      = 20000
BATCH_SIZE    = 64
...
class SymbolicRegressionEnv:
    def __init__(self, allowed_operators=None, x_range=X_RANGE, n_points=N_POINTS, max_length=MAX_LENGTH, load_case='constant'):
        ...
        self.load_case = load_case  # NEU: 'constant', 'linear', oder 'quadratic'
        self.reset()  
...
# Ersetze die Funktion in SymbolicRegressionEnv
    ...
    def _calculate_physics_loss(self, const_override=None):
        try:
            ...
            u_fourth_sym = sp.diff(u_sym, x_sym, 4) # Für die DGL
            u_double_sym = sp.diff(u_sym, x_sym, 2) # Für die Randbedingungen
            ...
            with np.errstate(all='ignore'):
                # 1. DGL-Residual berechnen
                u_xxxx_vals = u_xxxx_func(x_values_norm)

                # --- GEÄNDERT: Logik für die verschiedenen Lastfälle ---
                if self.load_case == 'constant':
                    # DGL: u'''' + 1 = 0
                    residual = u_xxxx_vals + 1.0
                elif self.load_case == 'linear':
                    # DGL: u'''' + x = 0
                    residual = u_xxxx_vals + x_values_norm
                elif self.load_case == 'quadratic':
                    # DGL: u'''' + x^2 = 0
                    residual = u_xxxx_vals + x_values_norm**2
                elif self.load_case == 'sinus':
                    # DGL: u'''' + sin(pi*x) = 0
                    residual = u_xxxx_vals + np.sin(np.pi * x_values_norm)
                else:
                    raise ValueError(f"Unbekannter Lastfall: {self.load_case}")
                # --------------------------------------------------------
...
# -------------------------------------------------
class DSPTrainer:
    def __init__(self, allowed_operators=None, episodes=EPISODES, lr=LR, load_case='constant'):
        self.env = SymbolicRegressionEnv(
            allowed_operators=allowed_operators, 
            load_case=load_case  # GEÄNDERT: load_case an die Umgebung übergeben
        )
        ...
        self.steps_done = 0
        self.load_case = load_case  # NEU: Lastfall im Trainer speichern
    ...
    def train(self, success_mse_threshold=1e-5): # NEU: Parameter für Erfolgsschwelle
        for episode in enhanced_progress_bar(range(1, self.episodes + 1), desc="Training läuft", unit=" Ep."):
            ...
            if expr:
                ...
                if self.top_solutions:
                    best_expr_so_far = self.top_solutions[0][1]
                    # Wichtig: calculate_mse muss den load_case kennen!
                    current_mse = calculate_mse(best_expr_so_far, self.load_case)
                    if current_mse < success_mse_threshold:
                        print_highlighted(f"\nErfolgreiche Lösung in Episode {episode} gefunden (MSE: {current_mse:.2e}). Breche Lauf ab.", Fore.GREEN)
                        return self.top_solutions, True # Gibt Erfolg zurück
        ...
        # GEÄNDERT: Wenn die Schleife ohne Erfolg durchläuft
        return self.top_solutions, False
...
# -------------------------------------------------
def analytic_solution(x_vals, load_case='constant'):
    # Das Vorzeichen wird an die DGL u'''' = -q/EI angepasst, sodass die Durchbiegung negativ ist.
    if load_case == 'constant':
        return -(q / (24 * E * I)) * (x_vals**4 - 2 * L * x_vals**3 + L**3 * x_vals)
    elif load_case == 'linear':
        # Analytische Lösung für q(x) = q_0 * x
        return -(q / (360 * E * I)) * (3 * x_vals**5 - 10 * L**2 * x_vals**3 + 7 * L**4 * x_vals)
    elif load_case == 'quadratic':
        # Analytische Lösung für q(x) = q_0 * x^2
        return -(q / (360 * E * I)) * (x_vals**6 - 5 * L**3 * x_vals**3 + 4 * L**5 * x_vals)
    elif load_case == 'sinus':
        # Analytische Lösung für q(x) = q_0 * sin(pi*x/L)
        return -(q * L**4) / (E * I * np.pi**4) * np.sin(np.pi * x_vals / L)
    else:
        raise ValueError(f"Unbekannter Lastfall für analytische Lösung: {load_case}")
...
def plot_reward_history(history, run_name):
    """Reward history (consistent fonts): blue = per-episode, red = moving average."""
    plt.figure(figsize=(12, 6))
    plt.plot(history, label='Reward per Episode', alpha=0.5)
    if len(history) > 100:
        mv = moving_average(history, 100)
        plt.plot(np.arange(99, len(history)), mv, color='red', linewidth=2, label='Moving Avg (100 eps)')
...
def plot_action_prob_heatmap(action_probs, epochs, env, run_name):
    """Action probability heatmap (consistent fonts, English labels)."""
    if not action_probs:
        return
    prob_array = np.array(action_probs)
    if prob_array.shape[0] <= 1:
        print(f"Not enough data for heatmap in {run_name}")
        return
    ...
    # Show y-ticks only every 1000 episodes (if applicable)
    if isinstance(epochs, (list, tuple, np.ndarray)):
        y_pos = [i for i, ep in enumerate(epochs) if (isinstance(ep, (int, np.integer)) and ep % 1000 == 0)]
        y_lbls = [str(ep) for ep in epochs if (isinstance(ep, (int, np.integer)) and ep % 1000 == 0)]
        if y_pos:
            ax.set_yticks(y_pos)
...
def _uc_for_load_case(load_case):
    """Characteristic deflection scale factor u_c for each load case."""
    if load_case == 'constant':
        return (q * L**4) / (E * I)
    elif load_case == 'linear':
        return (q * L**5) / (E * I)
...
def plot_result(expr_str, run_name, load_case='constant'):
    """
    Compare analytical vs SR solution (consistent fonts), do NOT print the expression in the legend,
    and print the dimensional SR expression to console (depends on load_case).
    """
    # Prepare data
    x_np = np.linspace(X_RANGE[0], X_RANGE[1], 200)
    y_analytic = analytic_solution(x_np, load_case)

    y_pred = np.full_like(x_np, np.nan)
    if expr_str:
        try:
            x_sym = sp.Symbol('x')
            ubar_sym = sp.sympify(expr_str.replace('^', '**'))
            ubar = sp.lambdify(x_sym, ubar_sym, 'numpy')

            x_bar = x_np / L
            u_c = _uc_for_load_case(load_case)
            y_pred = ubar(x_bar) * u_c

            # Console: dimensional SR expression u(x)
            u_dim_sym = sp.simplify(u_c * ubar_sym.subs(x_sym, x_sym / L))
            print_highlighted("Dimensional SR expression u(x):", Fore.CYAN)
            print(f"  load case = {load_case}")
            print(f"  u(x) = {u_dim_sym}")

        except Exception as e:
            print(f"Error evaluating SR solution for plot: {e}")

    # Plot solutions (no formula string in legend)
    plt.figure(figsize=(12, 7))
    plt.plot(x_np, y_analytic, 'k-', label='Analytical solution', linewidth=3)
    if expr_str:
        plt.plot(x_np, y_pred, 'r--', label='SR solution', linewidth=2)
    ...
    plt.plot(x_np, error, 'b-', linewidth=2)
    plt.title(f'Absolute Error – {run_name}', fontsize=24)
    plt.xlabel('x [m]', fontsize=22)
...
def calculate_mse(expr_str, load_case='constant'): # GEÄNDERT: load_case Parameter
    if not expr_str: return float('inf')
    try:
        # 1. Erstelle die x-Werte und die exakte, dimensionale Lösung
        x_np = np.linspace(X_RANGE[0], X_RANGE[1], 200)
        y_analytic = analytic_solution(x_np, load_case) # GEÄNDERT: Ruft korrekte Lösung auf
        ...
        # GEÄNDERT: Der Skalierungsfaktor u_c ist auch vom Lastfall abhängig
        if load_case == 'constant':
            u_c = (q * L**4) / (E * I)
        elif load_case == 'linear':
            u_c = (q * L**5) / (E * I) # q ist hier q_0
        elif load_case == 'quadratic':
            u_c = (q * L**6) / (E * I) # q ist hier q_0
        elif load_case == 'sinus':
            u_c = (q * L**4) / (E * I)
        else:
             u_c = 0 # Fallback
...
if __name__ == "__main__":
    ...
    NUM_RUNS = 1 # Anzahl der Wiederholungen pro Experiment
    SUCCESS_MSE_THRESHOLD = 1e-8 # Erfolgsschwelle für einen Lauf

    # Erweiterte Konfiguration für alle Experimente
    # Format: (Name des Experiments, erlaubte Operatoren, Lastfall-String)
    EXPERIMENT_CONFIGS = [
        # --- Konstanter Lastfall ---
        ("Constant load, All operators", ['+', '-', '*', '^', 'sin', 'cos', 'exp','/'], 'constant'),
        # --- Linearer Lastfall ---
        #("Linearer Lastfall, Polynom Ops", ['+', '-', '*'], 'linear'),
        # --- Quadratischer Lastfall ---
        #("Quadratischer Lastfall, Polynom Ops", ['+', '-', '*'], 'quadratic'),
        # --- HIER HINZUFÜGEN ---
        #("Sinus-Lastfall", ['+', '-', '*', 'sin'], 'sinus'),
    ]
    
    # ... der Rest des main-Blocks bleibt unverändert ...
    
    # Umfassendes Dictionary zum Speichern aller Endergebnisse
    all_results = {}
    ...
    param_data = [
        ...
        ["Basis-Streckenlast (q0)", f"{q:.2e} N/m"],
        ["Balkenlänge (L)", f"{L} m"],
        ["Anzahl Wiederholungen pro Experiment", f"{NUM_RUNS}"],
        ["Erfolgs-MSE-Schwelle", f"{SUCCESS_MSE_THRESHOLD:.1e}"],
    ]
    print_table(param_data)

    # Hauptschleife über alle definierten Experiment-Konfigurationen
    for name, operators, load_case in EXPERIMENT_CONFIGS:
        print_highlighted(f"\n{'='*25}\n=== STARTE EXPERIMENT: {name} ===\n{'='*25}", Fore.CYAN)
        
        successful_runs_count = 0
        best_solution_of_all_runs = {'expr': None, 'mse': float('inf')}
        
        # Schleife für die N Wiederholungen
        for i in range(1, NUM_RUNS + 1):
            print(f"\n--- {name} | Lauf {i}/{NUM_RUNS} ---")
            
            # Für jeden Lauf einen neuen Trainer instanziieren
            trainer = DSPTrainer(allowed_operators=operators, episodes=EPISODES, load_case=load_case)
            
            # Ein Training ausführen und auf Erfolg prüfen
            top_solutions, was_successful = trainer.train(success_mse_threshold=SUCCESS_MSE_THRESHOLD)

            # === GEWÜNSCHTE ÄNDERUNG START ===
            # 1. Erfolgszähler separat behandeln
            if was_successful:
                successful_runs_count += 1

            # 2. Immer die beste Lösung des aktuellen Laufs prüfen, unabhängig vom Erfolg
            if top_solutions:
                # Beste Lösung dieses einzelnen Laufs holen
                best_expr_this_run = top_solutions[0][1]
                mse_this_run = calculate_mse(best_expr_this_run, load_case)
                
                # Prüfen, ob sie besser ist als die bisher beste Lösung über alle Läufe
                if mse_this_run < best_solution_of_all_runs['mse']:
                    best_solution_of_all_runs['expr'] = best_expr_this_run
                    best_solution_of_all_runs['mse'] = mse_this_run
            # === GEWÜNSCHTE ÄNDERUNG ENDE ===
        
        # Speichere die aggregierten Ergebnisse für diese Konfiguration
        all_results[name] = {
            'success_rate': f"{successful_runs_count}/{NUM_RUNS}",
            'best_solution': best_solution_of_all_runs['expr'],
            'best_mse': best_solution_of_all_runs['mse'],
            'load_case': load_case
        }
        
        # Plotte das beste Ergebnis für diese Konfiguration
        if best_solution_of_all_runs['expr']:
            print_highlighted(f"\nBeste gefundene Lösung für '{name}': {best_solution_of_all_runs['expr']} (MSE: {best_solution_of_all_runs['mse']:.2e})", Fore.GREEN)
            # Deaktivieren der Analyse-Plots pro Lauf, um die Übersicht zu wahren
            # plot_reward_history(trainer.history, name)
            # plot_entropy_history(trainer.history, name)
            plot_result(best_solution_of_all_runs['expr'], name, load_case)

    # --- FINALE ZUSAMMENFASSUNG ALLER EXPERIMENTE ---
    print("\n\n" + "="*80)
    print_highlighted("=== GESAMTZUSAMMENFASSUNG ALLER EXPERIMENTE ===", Fore.MAGENTA)
    print("="*80)
    
    summary_data = []
    headers = ["Experiment", "Erfolgsrate", "Beste gefundene Lösung", "Bester MSE"]
    for name, res in all_results.items():
        summary_data.append([
            name, 
            res['success_rate'],
            res['best_solution'] if res['best_solution'] else "Keine gefunden",
            f"{res['best_mse']:.2e}" if res['best_mse'] != float('inf') else "N/A"
        ])
