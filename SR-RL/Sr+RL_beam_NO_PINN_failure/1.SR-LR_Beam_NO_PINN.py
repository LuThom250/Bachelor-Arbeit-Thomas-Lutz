...
# -------------------------------------------------
# "Normalisierte" Parameter für eine einfachere Lernaufgabe
E = 150e9
I = 40-6
W = 60e3  # W/(24*E*I) wird zu 1.0
L = 2.5   # Eine einfachere Länge
bc_weight = 0.5
...
# -------------------------------------------------
MAX_LENGTH    = 15      # Etwas mehr Länge für das Polynom 4. Grades
X_RANGE       = (0, L)
N_POINTS      = 50
EPISODES      = 20000
BATCH_SIZE    = 64
...
class SymbolicRegressionEnv:
    ...
    def _calculate_physics_loss(self, const_override=None):
        try:
            ...
            # Benötigte Ableitungen für den Balken
            u_double_sym = sp.diff(u_sym, x_sym, 2) # Für Randbedingungen u''(0) und u''(L)
            u_fourth_sym = sp.diff(u_sym, x_sym, 4) # Für die DGL u''''
            ...
            with np.errstate(all='ignore'):
                ...
                u_xxxx_vals = u_xxxx_func(self.x_values_np)
                residual = E * I * u_xxxx_vals + W # Neue DGL

                # 2. Randbedingungen auswerten
                u0 = u_func(0.0)
                uL = u_func(L)
                u0_dd = u_dd_func(0.0)
                uL_dd = u_dd_func(L)
            ...
            if u_xxxx_vals is None or np.any(np.isnan(residual)) or np.any(np.isinf(residual)) or \
               math.isnan(u0) or math.isnan(uL) or math.isnan(u0_dd) or math.isnan(uL_dd):
                return 1e10

            # 3. Verlust berechnen
            mse_res = float(np.mean(residual**2))
            # Vier Randbedingungen für den Balken
            bc_loss = u0**2 + uL**2 + u0_dd**2 + uL_dd**2
    ...
    def _calculate_reward(self, expr_str):
        ...
        # Skalierung basierend auf der Last, um den Loss in einen sinnvollen Bereich zu bringen
        scale = W**2 + 1e-6
    ...
    def forward(self, seq):
        ...
        if (lengths == 0).any():
            ...
            if (lengths > 0).any():
                ...
                embedded = self.embed(t[non_empty_mask])
                packed = nn.utils.rnn.pack_padded_sequence(embedded, lengths[non_empty_mask].cpu(), batch_first=True, enforce_sorted=False)
...
def analytic_solution(x_vals):
    # Analytische Lösung für den beidseitig gelagerten Balken
    return (W / (24 * E * I)) * (-x_vals**4 + 2 * L * x_vals**3 - L**3 * x_vals)
...
def plot_action_prob_heatmap(action_probs, epochs, env, run_name):
    ...
    plt.figure(figsize=(14, 8)); sns.heatmap(np.array(action_probs), cmap='viridis', xticklabels=action_labels, yticklabels=epochs)
    plt.title(f'Heatmap der Aktionswahrscheinlichkeiten (Initialzustand) - {run_name}'); plt.xlabel('Aktion (Token)'); plt.ylabel('Episode'); plt.show()
...
if __name__ == "__main__":
    EXPERIMENT_CONFIGS = [
        ("Balken - Polynom-Operatoren", ['+', '-', '*']),
        # Da die Lösung polynomial ist, sind andere Läufe weniger relevant
        # ("Balken - Lineare Operatoren", ['+', '-', '*']),
    ]
    results = {}
