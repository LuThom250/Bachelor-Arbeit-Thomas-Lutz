...
def plot_result(expr_str, run_name):
    """
    Zeichnet analytische Lösung und – falls vorhanden – gefundene SR-Lösung.
    Ausdruck wird nur in der Konsole ausgegeben.
    """
    x_np = np.linspace(X_RANGE[0], X_RANGE[1], 200)
    y_analytic = analytic_solution(x_np)

    # Ausdruck nur in der Konsole
    if expr_str:
    ...
    if expr_str:
        try:
            eta_func = sp.lambdify(sp.Symbol('x'),
...
if __name__ == "__main__":
    EXPERIMENT_CONFIGS = [
        ...
        #("Run4: All Operators", ['+', '-', '*','^', '/', 'sin', 'cos', 'exp']),
    ]
    results = {}
