...
# ==============================
# Wichtige Hyperparameter PINN (nach dem Vorbild des Balkens)
# ==============================
config = {
    "layers": [1, 8, 16, 16, 8, 1], # Angepasste Architektur für das einfachere Problem
    "N_f":  50,                    # Anzahl der Innen-Stützpunkte
    "lr":   1e-3,                  # Lernrate
    "n_epochs": 50000,             # Anzahl der Trainings-Epochen
    "min_error": 1e-5,             # Abbruchkriterium
}

def ada_max_update(params, grads, m, v, t,
                   beta1=0.9, beta2=0.999, alpha=config['lr'], eps=1e-8):
    """
...
def run_rod_pinn_simulation():
    """
    Führt die PINN-Simulation für den Stab mit CasADi aus und gibt die Zielfunktion für RL zurück.
    """
    print_highlighted("Phase 1: Starte CasADi-basierte PINN-Simulation für den Stab...", Fore.CYAN)
    t0 = time.time()
    x = ca.SX.sym('x')

    # Analytische Lösung für den Stab (nur zur Veranschaulichung)
    DGL_analytic = -q0/(6*E*A) * x**3 + q0*L**2/(2*E*A) * x

    # Kollokationspunkte
    collocation_points = np.linspace(0, L, config['N_f'])

    # ---- PINN-Netzwerk definieren (generisch) ----
    sizes = config['layers']
    sym_vars = []
    for i in range(len(sizes) - 1):
        sym_vars.append(ca.SX.sym(f'w{i+1}', sizes[i], sizes[i+1]))
        sym_vars.append(ca.SX.sym(f'b{i+1}', 1, sizes[i+1]))

    def network_forward(inp, params):
        res = inp
        for i in range(0, len(params), 2):
            w, b = params[i], params[i+1]
            res = res @ w + b
            if i < len(params) - 2: # Keine Aktivierung auf der letzten Schicht
                res = ca.tanh(res)
        return res

    dNN = network_forward(x, sym_vars)

    # Ableitungen des NN für das Stab-Problem
    d1NN = ca.jacobian(dNN, x)
    d2NN = ca.jacobian(d1NN, x)

    # ---- Zielfunktion für den Stab aufbauen ----
    # 1. Kollokationsfehler (E*A*u'' + q0*x = 0)
    col_err = 0
    for xi in collocation_points:
        residual = E * A * ca.substitute(d2NN, x, xi) + q0 * xi
        col_err += residual**2

    # 2. Randbedingungen (u(0) = 0, u'(L) = 0)
    bc_err = (
        ca.substitute(dNN,  x, 0)**2 +
        ca.substitute(d1NN, x, L)**2
    )

    ZF_sum = col_err / config['N_f'] + bc_err

    # ---- CasADi-Funktionen erstellen ----
    f_ZF = ca.Function('f_ZF', sym_vars, [ZF_sum])
    grads = [ca.reshape(ca.jacobian(ZF_sum, v), v.size1(), v.size2()) for v in sym_vars]
    f_grads = ca.Function('f_grads', sym_vars, grads)

    # ---- Parameter initialisieren und Training ----
    vals = [ca.DM.rand(*v.size()) * 0.1 for v in sym_vars]
    m = [ca.DM.zeros(*v.size()) for v in sym_vars]
    v = [ca.DM.zeros(*v.size()) for v in sym_vars]

    print("Starte PINN-Training...")
    for t in range(1, config['n_epochs'] + 1):
        zf_val = float(f_ZF(*vals)[0])
        if t % 1000 == 0:
            print(f"Epoch {t:6d}, Loss = {zf_val:.3e}")
        if zf_val <= config['min_error']:
            print(f"Training bei Epoch {t} beendet, da min_error erreicht wurde.")
            break
        grad_vals = f_grads(*vals)
        ada_max_update(vals, grad_vals, m, v, t)
    
    print(f"PINN-Training abgeschlossen. Endgültiger Loss: {zf_val:.3e}")
    print(f"Ausführungszeit PINN: {time.time() - t0:.2f}s")

    # ---- Ergebnisse auswerten und Zielfunktion für RL erstellen ----
    f_u = ca.Function('f_u', [x] + sym_vars, [dNN])
    x_dense = np.linspace(0, L, 200)
    u_dense = np.array([float(f_u(xi, *vals)[0]) for xi in x_dense]).flatten()

    # Plot der PINN-Lösung
    plt.figure(figsize=(8,5))
    plt.plot(x_dense, u_dense, label='CasADi PINN Output', linewidth=2)
    plt.xlabel('$x$')
    plt.ylabel('$u(x)$')
    plt.title('Approximierte Verschiebung des Stabs mit CasADi-PINN')
    plt.legend()
    plt.grid(True)
    plt.show()

    # Erstelle und gebe die Interpolationsfunktion zurück
    target_fn = interp1d(x_dense, u_dense, kind='cubic', fill_value="extrapolate")
    print_highlighted("PINN-Simulation abgeschlossen. `target_fn` für RL-Phase wurde erstellt.", Fore.GREEN)
    return target_fn


# === HAUPTPROGRAMM ===
# Phase 1: PINN-Simulation ausführen, um die Zieldaten zu generieren
target_fn = run_rod_pinn_simulation()
...
def plot_reward_history(history, run_name):
    """Plottet den Reward-Verlauf mit großer Schrift (Englisch) und Legende oben rechts."""
...
def plot_action_prob_heatmap(action_probs, epochs, env, run_name):
    ...
    plt.title(f'Action Probability Heatmap - {run_name}', fontsize=24)
    plt.xlabel('Action (Token)', fontsize=22)
    plt.ylabel('Episode', fontsize=22)
    plt.xticks(fontsize=20, rotation=45)

    # Nur jede 1000. Episode auf y-Achse anzeigen
    ax = plt.gca()
    y_positions = [i for i, ep in enumerate(epochs) if ep % 1000 == 0]
    y_labels_filtered = [str(epochs[i]) for i in y_positions]
    ax.set_yticks(y_positions)
    ax.set_yticklabels(y_labels_filtered, fontsize=14)

    # Colorbar größer machen
    cbar = plt.gcf().axes[-1]
    cbar.tick_params(labelsize=16)
    cbar.set_ylabel('Probability', fontsize=20, rotation=270, labelpad=20)
...
if __name__ == "__main__":
    TARGET_FUNCTIONS = [
        ("Run 1: All Operators", target_fn, ['+', '-', '*', '^', 'sin', 'cos', 'exp']),
        #("Run 2: Ohne exp", target_fn, ['+', '-', '*', '^', 'sin', 'cos']),
        #("Run 3: Nur Polynome", target_fn, ['+', '-', '*', '^']),
        ("Run 4: Linear operators", target_fn, ['+', '-', '*']),
    ]
