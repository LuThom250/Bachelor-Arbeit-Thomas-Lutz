...
class SymbolicRegressionEnv:
    ...
    # In der Klasse SymbolicRegressionEnv
    def _calculate_physics_loss(self, const_override=None):
        try:
            ...
            with np.errstate(all='ignore'):
                # u_dd_vals ist jetzt eta''(xi)
                u_dd_vals = u_dd_func(x_values_norm)
                # Das Residual für die DGL: eta''(xi) = -xi  =>  eta''(xi) + xi = 0
                residual = u_dd_vals + x_values_norm

                # Randbedingungen für eta(xi) auswerten
                u0 = u_func(0.0)      # für eta(0) = 0
                u1_d = u_d_func(1.0)  # für eta'(1) = 0

            # Auf ungültige Werte prüfen (bleibt fast gleich)
            if u_dd_vals is None or np.any(np.isnan(residual)) or np.any(np.isinf(residual)) or np.iscomplexobj(residual) or \
            np.iscomplex(u0) or np.iscomplex(u1_d) or math.isnan(u0) or math.isnan(u1_d): return 1e10
    ...
    def _calculate_reward(self, expr_str): # expr_str ist hier nur ein Trigger
        ...
        # NEUE SKALIERUNG: Keine explizite Skalierung mehr nötig.
        # Die exp-Funktion ist eine gute Wahl für den jetzt kleinen Loss-Wert.
...
# In der MAIN-FUNKTION UND ANALYSE Sektion
...
def plot_result(expr_str, run_name):
    ...
    if expr_str:
        try:
            # eta_func ist die gefundene dimensionslose Lösung eta(xi)
            eta_func = sp.lambdify(sp.Symbol('x'), sp.sympify(expr_str.replace('^', '**')), 'numpy')
            
            # Um zu plotten, skalieren wir das Ergebnis zurück
            xi_np = x_np / L  # x -> xi
            eta_pred = eta_func(xi_np)  # eta = f(xi)
            
            # Charakteristische Verschiebung für den Stab zum Zurückskalieren
            U = (q0 * L**3) / (E * A)
            y_pred = eta_pred * U  # u = eta * U
    ...
    plt.figure(figsize=(12, 5)); plt.plot(x_np, error, 'b-'); plt.title(f'Absoluter Fehler - {run_name}'); plt.xlabel('$x$'); plt.ylabel('Absoluter Fehler'); plt.grid(True, alpha=0.5); plt.show()
...
def calculate_mse(expr_str):
    ...
    try:
        ...
        # Skaliere das Ergebnis zurück für den MSE-Vergleich
        xi_np = x_np / L
        eta_pred = eta_func(xi_np)
...
if __name__ == "__main__":
    EXPERIMENT_CONFIGS = [
        ("Lauf 1: linear-Operatoren", ['+', '-', '*']),
        ("Lauf 2: polynom Operatoren", ['+', '-', '*','^']),
    ]
    results = {}
