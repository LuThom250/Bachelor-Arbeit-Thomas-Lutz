...
def plot_result(expr_str, run_name):
    """
    Compare analytical vs SR solution for the beam, print error progression and
    dimensional form of the best SR expression to console, and plot results.
    """
    # Generate x-values and analytical solution
    x_np = np.linspace(X_RANGE[0], X_RANGE[1], 200)
    y_analytic = analytic_solution(x_np)

    # Compute SR prediction (scaled back to dimensional u(x))
    ...
    # Console output of error progression
    print_highlighted("Error progression along the beam:", Fore.CYAN)
    for xi, ei in zip(x_np, error):
        print(f"  x = {xi:.3f} m → error = {ei:.3e} m")

    # Console output of the dimensional SR expression
    if expr_str:
        x = sp.Symbol('x')
        ubar_sym = sp.sympify(expr_str.replace('^', '**'))
        u_dim_sym = sp.simplify((q * L**4) / (E * I) * ubar_sym.subs(x, x/L))
        print_highlighted("Dimensional SR expression u(x):", Fore.CYAN)
        print(f"  u(x) = {u_dim_sym}")

    # Plot analytical vs SR
