    ...
    def _calculate_physics_loss(self, const_override=None):
        try:
            ...
            with np.errstate(all='ignore'):
                # 1. DGL-Residual berechnen: EI u'''' + q = 0
                u_xxxx_vals = u_xxxx_func(self.x_values_np)
                residual = E * I * u_xxxx_vals + q

                # 2. Randbedingungen auswerten (beidseitig gelenkig gelagert)
                u0 = u_func(0.0)
                uL = u_func(L)
                u0_dd = u_dd_func(0.0)
                uL_dd = u_dd_func(L)
            ...
            if u_xxxx_vals is None or np.any(np.isnan(residual)) or np.any(np.isinf(residual)) or \
               np.iscomplexobj(residual) or np.iscomplex(u0) or np.iscomplex(uL) or \
               np.iscomplex(u0_dd) or np.iscomplex(uL_dd) or math.isnan(u0) or math.isnan(uL) or \
               math.isnan(u0_dd) or math.isnan(uL_dd):
                return 1e10
...
def plot_reward_history(history, run_name):
    """Reward history with English labels and large fonts."""
    plt.figure(figsize=(12, 6))
    plt.plot(history, label='Reward per Episode', alpha=0.5)
    if len(history) > 100:
        avg = moving_average(history, 100)
        plt.plot(
            np.arange(99, len(history)), avg, color='red',
            linewidth=2, label='Moving Avg (100 eps)'
        )
...
def plot_action_prob_heatmap(action_probs, epochs, env, run_name):
    """Action probability heatmap with English labels and large fonts."""
    if not action_probs:
        return
    ...
    # show only every 1000th episode on y-axis
...
def plot_result(expr_str, run_name):
    """Compare analytical vs SR solution. Expr printed to console only."""
    x_np = np.linspace(X_RANGE[0], X_RANGE[1], 200)
    y_analytic = analytic_solution(x_np)

    # print expression in console
    if expr_str:
